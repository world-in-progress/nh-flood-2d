...
class CouplingTimer:
    ...
    def report(self, prefix: str = '[timer]') -> None:
        ...
        coupled = self.exchange_count > 0
        label = 'Coupling Statistics' if coupled else 'Simulation Statistics'
        step_2d = self.total_2d_step if coupled else elapsed
        print(f'{prefix} ────────────── {label} ──────────────')
        if coupled:
            print(f'{prefix}   Exchange windows completed : {self.exchange_count}')
        print(f'{prefix}   Total wall time            : {elapsed:.1f} s')
        print(f'{prefix}   2D step total              : {step_2d:.1f} s')
        if coupled:
