...
def prepare_pipe(
    ...
    # compute esl (side length) per element: use left-side x offset * 2
    # matches solver_compact.py L233: esl[ei] = (ex[ei] - sx[lsi0]) * 2
    # here we approximate from ne.fdb stored data
    from ..schema.feature import Ns
    ns_db = fdb.ORM.load(domain_cfg.ns_fdb, from_file=True)
    ns_len = ns_db[Ns][Ns].column.length.copy()

    isl_db = ne_db
    isl1_arr = isl_db[IndexLike]['isl1'].column.index.copy()
    isl_ptr_l = ne_db[IndexLike].get('isl_ptr_l', None)

    # fallback: use first left-side length as esl approximation
    esl_np = np.zeros(e_num, dtype=np.float32)
    l_side_nums = nes.column.l_side_num.copy()
    ptr_l = ne_db[IndexLike]['isl_ptr_l'].column.index.copy() if 'isl_ptr_l' in [t.name for t in ne_db.table_definitions] else None

    if ptr_l is not None:
        for ei in range(1, e_num):
            s = int(ptr_l[ei])
            if s < len(isl1_arr):
                si = int(isl1_arr[s])
                if si > 0 and si < len(ns_len):
                    esl_np[ei] = float(ns_len[si]) * max(int(l_side_nums[ei]), 1)
    else:
        # simple fallback: uniform esl from first element
        for ei in range(1, e_num):
            esl_np[ei] = 1.0  # will be overridden if ns data is available
    ...
    # ── write pipe.fdb ────────────────────────────────────────────────────────
    n_topo = max(len(topo_ei_flat), 1)

    db = fdb.ORM.truncate([
        fdb.TableDefn(Node,      n_nodes,        'Node'),
        fdb.TableDefn(IndexLike, n_nodes,         'node_primary_ei'),
        fdb.TableDefn(IndexLike, e_num,           'node_count_per_ei'),
        fdb.TableDefn(PipeTopo,  n_topo,          'PipeTopo'),
        fdb.TableDefn(IndexLike, n_nodes + 1,     'topo_ptr'),
        fdb.TableDefn(IndexLike, 1,               'node_count'),
    ])

    # node table
    nodes_t  = db[Node]['Node']
    for i, node in enumerate(node_list):
        row = nodes_t[i]
        row.index      = i
        row.name       = node.name
    ...
    # primary ei
    db[IndexLike]['node_primary_ei'].column.index[:] = primary_ei_arr.astype(np.uint32)

    # node count per ei
    db[IndexLike]['node_count_per_ei'].column.index[:] = nc_per_ei

    # topo (secondary)
    if len(topo_ei_flat) > 0:
        db[PipeTopo]['PipeTopo'].column.ei[:len(topo_ei_flat)] = topo_ei_flat

    # topo_ptr (0-based CSR offsets, length n_nodes+1)
    db[IndexLike]['topo_ptr'].column.index[:] = topo_ptr.astype(np.uint32)

    # node_count
    db[IndexLike]['node_count'][0].index = n_nodes

    db.save(pipe_cfg.pipe_fdb)
    print(f'[prepare_pipe] wrote {n_nodes} nodes, {len(topo_ei_flat)} secondary topo entries → {pipe_cfg.pipe_fdb}')
