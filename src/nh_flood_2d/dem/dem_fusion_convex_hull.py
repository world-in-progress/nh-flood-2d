...
# -*- coding: utf-8 -*-
"""
DEM融合脚本 - 将低精度区域DEM与高精度局部高程点云融合（凸包版本）
作者: 高级GIS工程师
日期: 2026-03-31

核心算法: 点云替换与统一插值法（基于凸包）
1. 读取区域DEM并展平为点云
2. 读取高精度点云，计算其凸包作为替换区域边界
3. 从DEM点云中剔除凸包内点（挖洞）
4. 合并剩余DEM点云与凸包内高精度点云
5. 构建TIN并重采样到固定4m分辨率网格
6. 输出融合后的DEM
...
def compute_convex_hull_from_points(points_xyz: np.ndarray, buffer_distance: float = 0.0) -> Polygon:
    """
    从点云计算凸包多边形

    Args:
        points_xyz: Nx3点云数组
        buffer_distance: 缓冲区距离（米），用于扩大凸包范围

    Returns:
        Polygon: 凸包多边形（可能带缓冲区）
    """
    logger.info("计算点云凸包")
    ...
    # 检查点云数量
    if len(xy_points) < 3:
        logger.warning("点云数量不足，无法计算凸包")
        # 创建一个小矩形作为替代
        minx, miny = xy_points.min(axis=0)
        maxx, maxy = xy_points.max(axis=0)
        ...
        logger.info(f"使用边界框替代凸包: {hull_polygon.bounds}")
        return hull_polygon
    ...
    try:
        ...
        # 创建凸包多边形（注意：shapely的Polygon需要闭合多边形，即第一个点和最后一个点相同）
        hull_polygon = Polygon(hull_vertices)

        # 应用缓冲区（如果指定）
        if buffer_distance > 0:
            original_area = hull_polygon.area
            hull_polygon = hull_polygon.buffer(buffer_distance)
            logger.info(f"凸包缓冲区: {buffer_distance}米，面积从 {original_area:.2f} 增加到 {hull_polygon.area:.2f} 平方米")

        logger.info(f"凸包面积: {hull_polygon.area:.2f} 平方米")
        logger.info(f"凸包边界: {hull_polygon.bounds}")
        logger.info(f"凸包顶点数: {len(hull_vertices)}")

        return hull_polygon

    except Exception as e:
        logger.error(f"计算凸包失败: {e}")
        # 如果凸包失败，使用边界框
        minx, miny = xy_points.min(axis=0)
        maxx, maxy = xy_points.max(axis=0)
        hull_polygon = Polygon([
...
def save_convex_hull_to_shapefile(polygon: Polygon, crs: str, output_path: str):
    """
    将凸包多边形保存为shapefile，用于可视化
    ...
    """
    logger.info(f"保存凸包到shapefile: {output_path}")
    ...
    try:
        ...
        logger.info(f"凸包shapefile保存成功: {output_path}")

    except Exception as e:
        logger.warning(f"保存凸包shapefile失败: {e}")


def fuse_dem_and_points_convex_hull(dem_path: str, points_path: str, output_path: str,
                                   points_skip_rows: int = 0, dem_sample_fraction: float = 1.0,
                                   hull_buffer_distance: float = 0.0, save_hull_shp: bool = True):
    """
    主函数：融合DEM与高精度点云（使用凸包）
    ...
    Args:
        ...
        hull_buffer_distance: 凸包缓冲区距离（米），用于扩大替换区域
        save_hull_shp: 是否保存凸包为shapefile用于可视化
    """
    start_time = time.time()
    logger.info("开始DEM融合处理（使用凸包）")
    logger.info(f"基础DEM: {dem_path}")
    logger.info(f"高精度点云: {points_path}")
    logger.info(f"输出文件: {output_path}")
    logger.info(f"凸包缓冲区距离: {hull_buffer_distance}米")

    try:
        # 步骤1: 读取高精度点云
        logger.info("步骤1/7: 读取高精度点云")
        ...
        # 步骤2: 计算高精度点云的凸包
        logger.info("步骤2/7: 计算高精度点云凸包")
        hull_polygon = compute_convex_hull_from_points(highres_points, hull_buffer_distance)

        # 可选：保存凸包为shapefile
        if save_hull_shp:
            hull_shp_path = output_path.replace('.tif', '_convex_hull.shp')
            # 需要先读取DEM获取CRS
            with rasterio.open(dem_path) as src:
                save_convex_hull_to_shapefile(hull_polygon, src.crs, hull_shp_path)

        # 步骤3: 筛选高精度点云中落在凸包内的点
        logger.info("步骤3/7: 筛选凸包内的高精度点")
        highres_points_in_hull = filter_points_inside_polygon(highres_points, hull_polygon)

        if len(highres_points_in_hull) == 0:
            logger.warning("没有高精度点落在凸包区域内，将使用原始DEM数据")
            # 如果没有高精度点，直接使用原始DEM
            with rasterio.open(dem_path) as src:
                dem_array = src.read(1)
                dem_meta = {
                ...
                logger.info("处理完成：直接复制原始DEM（没有高精度点落在凸包内）")
                return

        logger.info(f"凸包内高精度点: {len(highres_points_in_hull):,}个点")

        # 步骤4: 读取基础DEM点云
        logger.info("步骤4/7: 读取基础DEM点云")
        dem_points, dem_meta = extract_dem_points(dem_path, dem_sample_fraction)

        # 步骤5: 从DEM点云中剔除凸包内点（挖洞）
        logger.info("步骤5/7: 从DEM点云中挖洞（移除凸包内点）")
        filtered_dem_points = filter_points_by_polygon(dem_points, hull_polygon)

        # 步骤6: 合并点云
        logger.info("步骤6/7: 合并点云")
        if len(filtered_dem_points) > 0:
            merged_points = np.vstack([filtered_dem_points, highres_points_in_hull])
        else:
            merged_points = highres_points_in_hull

        logger.info(f"合并后点云总数: {len(merged_points):,}个点")
        logger.info(f"  - DEM点云（凸包外）: {len(filtered_dem_points):,}个点")
        logger.info(f"  - 高精度点（凸包内）: {len(highres_points_in_hull):,}个点")
...
def main():
    """主函数：示例调用"""
    # 设置文件路径
    dem_path = "fused_dem_result.tif"
    points_path = "bay/bay/shenzhenhe.txt"
    output_path = "fused_dem_convex_hull.tif"

    # 配置参数
    config = {
        'points_skip_rows': 0,          # 没有表头行
        'dem_sample_fraction': 0.02,    # 采样2%的DEM点（约240万点）
        'hull_buffer_distance': 10.0,   # 凸包缓冲区距离10米
        'save_hull_shp': True           # 保存凸包为shapefile
    }

    try:
        # 调用融合函数
        fuse_dem_and_points_convex_hull(
            dem_path=dem_path,
